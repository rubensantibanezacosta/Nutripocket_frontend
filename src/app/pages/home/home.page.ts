...
export class HomePage {
  ...
  calculateTotals(day: Day) {
    day.dishes.forEach((day) => {
      ...
      day.items.forEach((item) => {
        ...
        this.daytotals.saturatedFat += item.dosex * item.food.doseg * item.food.saturatedfat100g / 100;
        this.daytotals.sugar += item.dosex * item.food.doseg * item.food.sugar100g / 100;
        this.daytotals.fiber += item.dosex * item.food.doseg * item.food.fiber100g / 100;
        this.daytotals.salt += item.dosex * item.food.doseg * item.food.salt100g / 100;
    ...
    const caloriesReqPerMinute = this.evolution.caloriesburned / this.HOURS_IN_DAY / this.MINUTES_IN_HOUR || 0;
    ...
    this.dayService.updateDay(this.currentDay.id, this.currentDay).subscribe((res) => {
      return
    },
      error => console.error(error.message)
    )
  ...
  async regenerateDay() {
    ...
    confirm.onDidDismiss().then((response) => {
      if (response.data === true) {
        this.loading("Estamos calculando tu dieta de hoy, espera unos segundos...")
        this.dayService.regenerateDay(this.date).subscribe((data) => {
          this.getCurrentDay();
          this.setTotalsToZero();
          this.getLastEvolution();
  ...
  async editFoodWeight(item: Item, dayfood: number, dishid) {
    ...
    popover.onDidDismiss().then((itemReturned) => {
      ...
      if (itemReturned.data) {
        this.itemService.updateItem(item.id, <Item>itemReturned.data, dishid).subscribe(async (res) => {
          this.getCurrentDay();
          this.setTotalsToZero();
          this.getLastEvolution();
        })
  ...
  async generateWeek() {
    ...
    confirm.onDidDismiss().then((response) => {
      if (response.data === true) {
        this.loading("Estamos calculando tu dieta semanal, espera unos segundos...");
        this.dayService.generateWeek().subscribe((res) => {
          setTimeout(() => {
            this.getCurrentDay();
            this.setTotalsToZero();
            this.getLastEvolution();
